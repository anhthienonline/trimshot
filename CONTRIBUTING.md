...
- **`NSWindow.acceptsMouseMovedEvents` defaults to false.** Without it `mouseMoved:` is never
  ...
  the window opened. This shipped broken in the measure tool because `--render-chrome` sets
  the pointer directly — a harness that assigns state cannot catch state that is never assigned
  at runtime. Features that follow the cursor need a real click-through.
